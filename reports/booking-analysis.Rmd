...
```{r}
resampled_CI <- quantile(sample_means, probs = c(0.025, 0.975))
resampled_CI
```

The 95% CI of the bootstrapped means ranges from 942.4681 to 942.5221 (or from 15:42 to 15:43).
```{r}
mins_to_time(resampled_CI)
```

This means that we are 95% confident that the true average booking time falls between 15:41 and 15:44.
